...
module RestClient
  ...
  class Request
    ...
    # modified process_payload to accept nested arrays and hashes.
    # example: :machines => {:servers => [{:id => '1', :hostname => 'server-1'}, {:id => '2', :hostname => 'server-2'}]}
    # returns machines[servers][][id]
    # @@yey = [] is an awful hack, but I'd been trying for too long.
    @@yey = Array.new    
    def process_payload(p=nil, parent_key=nil)
      unless p.is_a?(Hash) or p.is_a? Array
        @@yey.push p
        p
      else
        ...
        p.keys.map do |k|
          ...
          if p[k].is_a? Array
            p[k].each do |valor|
              key = "#{parent_key}[#{k}][]"
              process_payload(valor, key)
            end
          end
          if p[k].is_a? Hash
            process_payload(p[k], key)
          else
           unless p[k].is_a? Array
